...
class SegmentationMultiProcess(object):
    ...
    def run(self, process_idx, cfg):
        ...
        if self.phase == 'test':
            ...
            if self.cfg.DATA_LOADER.IS_COARSE:
                ...
                self.model_1 = get_coarse_model(cfg, self.phase)
            else:
                self.model = get_fine_model(cfg, self.phase)
                self.model_1 = get_fine_model(cfg, self.phase)
        ...
        self.optimizer_1 = self._init_optimizer_1() if self.phase == 'train' else None
        # set apex training
        if self.is_apex_train:
            self.model, self.optimizer = amp.initialize(self.model, self.optimizer, opt_level='O1')
            self.model_1, self.optimizer_1 = amp.initialize(self.model_1, self.optimizer_1, opt_level='O1')
        # load model weight
        self._load_weights()
    ...
    def _init_optimizer(self):
        ...
        return optimizer
    def _init_optimizer_1(self):
        if self.cfg.TRAINING.OPTIMIZER.METHOD == 'sgd':
            optimizer = optim.SGD(self.model_1.parameters(), lr=self.cfg.TRAINING.OPTIMIZER.LR,
                                  momentum=0.99, weight_decay=self.cfg.TRAINING.OPTIMIZER.L2_PENALTY)
        elif self.cfg.TRAINING.OPTIMIZER.METHOD == 'adam':
            optimizer = optim.Adam(self.model_1.parameters(), lr=self.cfg.TRAINING.OPTIMIZER.LR,
                                   betas=(0.9, 0.99), weight_decay=self.cfg.TRAINING.OPTIMIZER.L2_PENALTY)
        return optimizer
    ...
    def _create_train_data(self):
        ...
        trainu_dataset = SegDataSet(self.cfg, 'trainu')
        val_dataset = SegDataSet(self.cfg, 'val')

        if self.is_distributed_train:
            train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
            trainu_sampler = torch.utils.data.distributed.DistributedSampler(trainu_dataset)
        ...
        self.trainu_loader = DataLoaderX(
            dataset=trainu_dataset,
            batch_size=self.cfg.DATA_LOADER.BATCH_SIZE,
            num_workers=self.num_worker,
            shuffle=True if trainu_sampler is None else False,
            drop_last=False,
            pin_memory=True,
            sampler=trainu_sampler)
    ...
    def do_train(self, fold=1):
        ...
        if self.is_print_out:
            ...
            self.trainu_writer = SummaryWriter(log_dir=os.path.join(self.log_dir, 'trainu'))
            self.val_writer = SummaryWriter(log_dir=os.path.join(self.log_dir, 'val'))

        if torch.cuda.device_count() > 1:
            if not self.is_distributed_train:
                self.model = torch.nn.DataParallel(self.model)
                self.model_1 = torch.nn.DataParallel(self.model_1)
            elif self.is_apex_train and self.is_distributed_train:
                self.model = DistributedDataParallel(self.model, delay_allreduce=True)
                self.model_1 = DistributedDataParallel(self.model_1, delay_allreduce=True)
            elif self.is_distributed_train:
                self.model = torch.nn.parallel.DistributedDataParallel(self.model,
                                                                       device_ids=[self.local_rank],
                                                                       output_device=self.local_rank)
                self.model_1 = torch.nn.parallel.DistributedDataParallel(self.model_1,
                                                                       device_ids=[self.local_rank],
                                                                       output_device=self.local_rank)
        ...
        for epoch in range(self.start_epoch, self.cfg.TRAINING.SCHEDULER.TOTAL_EPOCHS + 1):
            ...
            val_dice_1 = self._validate_1(epoch)
            if self.is_print_out:
    ...
    def _train(self, epoch):
        ...
        train_total = 0
        train_dice_1 = [0.] * len(self.cfg.DATA_LOADER.LABEL_INDEX)
        train_total_1 = 0
        nnnum=len(self.train_loader)
        for index, data_u_s in enumerate(zip(cycle(self.train_loader),self.trainu_loader)):
            #print(index)
            images, masks=data_u_s[0]
            images_u, masks_u=data_u_s[1]
            ...
            if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                ...
                output_seg_list_1 = self.model_1(images)
                seg_loss = 0
                ...
                # for i in output_1_u:
                # _, max_i=torch.max(i, dim=1)
                # max_i=torch.unsqueeze(max_i, dim=1)
                # max_1_u.append(max_i.long())
                seg_loss /= len(output_seg_list)
                output_seg = output_seg_list[0]
            else:
                # output_seg = self.model(images)
                # output_seg_1 = self.model_1(images)
                # output_seg_u = self.model(images_u)
                # output_seg_u_as_s= output_seg_u.long()
                # output_seg_1_u = self.model_1(images_u)
                # output_seg_1_u_as_s= output_seg_1_u.long()
                # loss_func = SegLoss(loss_func=self.cfg.TRAINING.LOSS, activation=self.cfg.TRAINING.ACTIVATION)
                
                # seg_loss_s = loss_func(output_seg, masks, is_average=True)
                # seg_loss_s_1 = loss_func(output_seg_1, masks, is_average=True)
                # seg_loss_u = loss_func(output_seg_u, output_seg_1_u_as_s, is_average=True)
                # seg_loss_u_1 = loss_func(output_seg_1_u, output_seg_u_as_s, is_average=True)
                # seg_loss=seg_loss_s+seg_loss_u
                # seg_loss_1=seg_loss_s_1+seg_loss_u_1
                ...
                pred=torch.cat([output_seg,output_seg_u],dim=0)
                pred_1=torch.cat([output_seg_1,output_seg_1_u],dim=0)


                # with torch.no_grad():
                #     shp_x = pred.shape
                #     max_pred = F.sigmoid(pred)
                #     max_pred_1 = F.sigmoid(pred_1)

                #     max_pred=max_pred.argmax(dim=1)
                #     max_pred_1=max_pred_1.argmax(dim=1)

                #     max_pred_onehot=F.one_hot(max_pred,shp_x[1]+1).float()
                #     max_pred_1_onehot=F.one_hot(max_pred_1,shp_x[1]+1).float()

                #     max_pred_onehot=max_pred_onehot.permute(0,4,1,2,3)
                #     max_pred_1_onehot=max_pred_1_onehot.permute(0,4,1,2,3)

                #     ind=[]
                #     for i in range(shp_x[1]):   # 取tensor的前200行
                #         ind.append(i+1)
                #     #y_onehot = y_onehot.cuda(x.device.index)
                #     indices = torch.LongTensor(ind).to(pred.device)
                #     max_pred_onehot = torch.index_select(max_pred_onehot, 1, indices).to(pred.device)

                #     ind_1=[]
                #     for i in range(shp_x[1]):   # 取tensor的前200行
                #         ind_1.append(i+1)
                #     #y_onehot = y_onehot.cuda(x.device.index)
                #     indices_1 = torch.LongTensor(ind_1).to(pred_1.device)
                #     max_pred_1_onehot = torch.index_select(max_pred_1_onehot, 1, indices_1).to(pred_1.device)
                ...
                max_pred_onehot=max_pred_onehot[:, 1:]
                max_pred_1_onehot=max_pred_1_onehot[:, 1:]
                seg_loss_s = loss_func(output_seg, masks, is_average=True)
                seg_loss_s_1 = loss_func(output_seg_1, masks, is_average=True)
                seg_loss_u = loss_func(pred, max_pred_1_onehot, is_average=True)
                seg_loss_u_1 = loss_func(pred_1, max_pred_onehot, is_average=True)
            ...
            if self.is_apex_train:
                ...
                with amp.scale_loss(seg_loss_1, self.optimizer_1) as scaled_loss_1:
            ...
            dice_output = dice_metric_func(output_seg, masks, activation=self.cfg.TRAINING.ACTIVATION,
                                        is_average=False)
            dice_output_1 = dice_metric_func(output_seg_1, masks, activation=self.cfg.TRAINING.ACTIVATION,
                                        is_average=False)
            if self.is_distributed_train:
                dice_output = self._reduce_tensor(dice_output.data)
                dice_output_1 = self._reduce_tensor(dice_output_1.data)
            for i, dice_tmp in enumerate(dice_output):
                train_dice[i] += float(dice_tmp.item())
            for i, dice_tmp in enumerate(dice_output_1):
                train_dice_1[i] += float(dice_tmp.item())
            train_total += len(images)

            if self.is_distributed_train:
                seg_loss = self._reduce_tensor(seg_loss.data)
                seg_loss_1 = self._reduce_tensor(seg_loss_1.data)
            ...
            if self.is_print_out:
                if index > 0 and (index) % self.cfg.TRAINING.SAVER.SAVER_FREQUENCY == 0:
                    self.logger.info('Epoch: {}/{} [{}/{} ({:.0f}%)]'.format(
                        ...
                        index * len(images), len(self.train_loader)+len(self.trainu_loader),
                        100. * index  / (len(self.train_loader)+len(self.trainu_loader))))
                    self.logger.info('SegLoss:{:.6f}'.format(seg_loss.item()))
                    self.logger.info('SegLoss_1:{:.6f}'.format(seg_loss_1.item()))
                    ...
                    for i, dice_label in enumerate(train_dice_1):
                        dice_ind = dice_label / train_total
                        self.logger.info('{} Dice_1:{:.6f}'.format(self.cfg.DATA_LOADER.LABEL_NAME[i], dice_ind))

        if self.is_print_out:
                self.train_writer.add_scalar('Train/SegLoss', seg_loss.item(), epoch)
                self.train_writer.add_scalar('Train/SegLoss_1', seg_loss_1.item(), epoch)
                for i, dice_label in enumerate(train_dice):
                    dice_ind = dice_label / train_total
                ...
                for i, dice_label in enumerate(train_dice_1):
                    dice_ind = dice_label / train_total
                    self.train_writer.add_scalars('Train/Dice_1',
                                                {self.cfg.DATA_LOADER.LABEL_NAME[i]: dice_ind}, epoch)
    ...
    def _validate_1(self, epoch):
        ...
        for images, masks in self.val_loader:
            ...
            with torch.no_grad():
                if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                    output_seg_list = self.model_1(images)
                    output_seg = output_seg_list[0]
                else:
                    output_seg = self.model_1(images)
        ...
        total_dice = 0
        if self.is_print_out:
            self.logger.info('Loss of validation_1 is {}'.format(val_loss))
            self.val_writer.add_scalar('Val_1/Loss', val_loss, epoch)

            for idx, _ in enumerate(val_dice):
                val_dice[idx] /= val_total
                self.logger.info('{} Dice_1:{:.6f}'.format(self.cfg.DATA_LOADER.LABEL_NAME[idx], val_dice[idx]))
                self.val_writer.add_scalars('Val_1/Dice',
                                            {self.cfg.DATA_LOADER.LABEL_NAME[idx]: val_dice[idx]}, epoch)
                total_dice += val_dice[idx]
                if is_hausdorff:
                    val_hausdorff[idx] /= val_total
                    self.logger.info('{} Hausdorff_1:{:.6f}'.format(self.cfg.DATA_LOADER.LABEL_NAME[idx],
                                                                  val_hausdorff[idx]))
                    self.val_writer.add_scalars('Val_1/Hausdorff',
                                                {self.cfg.DATA_LOADER.LABEL_NAME[idx]: val_hausdorff[idx]}, epoch)
                    total_dice += val_hausdorff[idx]
            total_dice /= len(val_dice)
            if is_hausdorff:
