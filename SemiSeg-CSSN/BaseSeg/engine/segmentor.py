...
class BaseSegmentation3D(object):
    ...
    def __init__(self, cfg, phase='train'):
        ...
        if self.is_apex_train:
            ...
            self.model_1, self.optimizer_1 = amp.initialize(self.model_1, self.optimizer_1, opt_level='O0')
    ...
    def _save_weights(self, epoch, net_state_dict, optimizer_state_dict,num_model):
        if self.is_print_out:
            model_dir = os.path.join(self.train_save_dir, 'models')
            if not os.path.exists(model_dir):
                os.makedirs(model_dir)
            self.best_model_weight_path = os.path.join(model_dir, 'best_model'+num_model+'.pt')
    ...
    def do_train(self):
        ...
        for epoch in range(self.start_epoch, self.cfg.TRAINING.SCHEDULER.TOTAL_EPOCHS + 1):
            ...
            if self.is_print_out:
                ...
                if val_dice_1 > best_dice_1:
                    best_dice_1 = val_dice_1
                    self._save_weights(epoch, self.model_1.state_dict(), self.optimizer_1.state_dict(),'_1')

                self.logger.info('End of epoch {}, time: {}'.format(epoch, datetime.datetime.now() - start_time))

        if self.is_print_out:
            self.train_writer.close()
            self.trainu_writer.close()
    ...
    def covert2onehot(self, x,y):
        ...
        with torch.no_grad():
            ...
            y = y.long()
            y_onehot = torch.zeros((shp_x[0], shp_x[1]+1, *shp_x[2:]))
    ...
    def _train(self, epoch):
        ...
        self.label = self.cfg.DATA_LOADER.LABEL_INDEX
        ...
        for index, data_u_s in enumerate(zip(self.train_loader,cycle(self.trainu_loader))):
            #print(index)
            ...
            if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                ...
                _,max_pred_1=torch.max(pred_1,dim=1)

                #将max_pred转成one_hot形式
                max_pred_onehot=self.covert2onehot(pred_1,max_pred)
                max_pred_1_onehot=self.covert2onehot(pred,max_pred_1)
                max_pred_onehot=max_pred_onehot[:, 1:]
                max_pred_1_onehot=max_pred_1_onehot[:, 1:]

                seg_loss_cps= loss_func(pred, max_pred_1_onehot, is_average=True)
                seg_loss_cps_1=loss_func(pred_1, max_pred_onehot, is_average=True)
                seg_loss_s = loss_func(output_seg, masks, is_average=True)
                seg_loss_s_1=loss_func(output_seg_1, masks, is_average=True)
                
                seg_loss=seg_loss_s+seg_loss_cps
                seg_loss_1=seg_loss_s_1+seg_loss_cps_1
            if self.is_apex_train:
                with amp.scale_loss(seg_loss, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
                with amp.scale_loss(seg_loss_1, self.optimizer_1) as scaled_loss:
                    scaled_loss.backward()
            else:
                seg_loss.backward()
                seg_loss_1.backward()
