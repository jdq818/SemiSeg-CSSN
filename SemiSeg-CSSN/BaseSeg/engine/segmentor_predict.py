...
from Common.mask_process import extract_bbox, extract_bbox_center,extract_bbox_center_and_mask,crop_image_according_to_bbox, extract_topk_largest_candidates
...
class SegmentationInfer(object):
    ...
    def respacing_itk(self,nrrd_dir, interp_type, new_spacing, patient_id, return_type, save_dir): 
        ...
        if return_type == 'nrrd':
            ...
            return img_arr
    def preprocess(self):
        image_dir = self.cfg.DATA_LOADER.TEST_IMAGE_DIR
        data_filenames = os.listdir(image_dir)
        filenames = []
        test_folder=self.cfg.DATA_LOADER.TEST_IMAGE_DIR
        for file_name in data_filenames:
                series_id = file_name.split('_0000.nii.gz')[0]
                filenames.append(file_name)

        for filename in filenames:
            ori_image=os.path.join(test_folder, filename)
            series_id = filename.split('_0000.nii.gz')[0]
            save_file=os.path.join(test_folder,filename.replace('s_0','s_5'))
            if not os.path.exists(save_file):
                img_arr = self.respacing_itk(
                        nrrd_dir=ori_image,
                        interp_type='linear',
                        new_spacing=[2,2,2],
                        patient_id=None,
    ...
    def run(self):
        ...
        for batch_idx, data_dict in enumerate(test_dataloader):
            ...
            coarse_bbox_m = extract_bbox(out_coarse_mask)
            coarse_bbox_c=extract_bbox_center(out_coarse_mask,coarse_spacing)
            coarse_bbox=extract_bbox_center_and_mask(out_coarse_mask,coarse_spacing)
