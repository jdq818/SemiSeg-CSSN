...
from Common.image_io import load_ct_info,load_ct_info_v1, save_ct_from_npy
...
def run_prepare_data(cfg):
    ...
    for data in data_prepare.datau_info:
        try:
...
class DataPrepare(object):
    def __init__(self, cfg):
        ...
        self._create_db_file_v1(phase='train')
        #self._create_db_file(phase='train')
        self._create_db_file_v1(phase='test')
        self.data_info = self._read_db()
        self.datau_info = self._read_db_u()
    ...
    def process(self, data):
        ...
        if data.mask_path=='None':
            mask_info = load_ct_info_v1(data.mask_path,data.image_path)
        else:
        ...
        if data.mask_path=='None':
            t_crop_image, t_crop_mask = npy_image, npy_mask
        else:
    ...
    def _read_db_u(self):
        local_data = []
        env = lmdb.open(self.trainu_db_file, map_size=int(1e9))
        txn = env.begin()
        for key, value in txn.cursor():
            key = str(key, encoding='utf-8')
            value = str(value, encoding='utf-8')
        ...
        print('Num of unlab ct is %d.' % len(local_data))
        return local_data
    ...
    def _update_db(self, data):
        ...
        txn.commit()
        env.close()
    def _update_db_u(self, data):
        env = lmdb.open(self.outu_db_file, map_size=int(1e9))
        txn = env.begin(write=True)
    ...
    def _create_db_file_v1(self, phase='train'):
        db_file_path = self.train_db_file if phase == 'train' else self.test_db_file
        DataBaseUtils.creat_db(db_file_path)
        dbu_file_path = self.trainu_db_file if phase == 'train' else self.test_db_file
        DataBaseUtils.creat_db(dbu_file_path)
        series_ids = read_txt(self.cfg.DATA_PREPARE.TRAIN_SERIES_IDS_TXT) \
            if phase == 'train' else read_txt(self.cfg.DATA_PREPARE.TEST_SERIES_IDS_TXT)
        series_ids_u =read_txt(self.cfg.DATA_PREPARE.TRAINU_SERIES_IDS_TXT)
        for series_id in series_ids:
            if phase == 'train':
                image_path = os.path.join(self.cfg.DATA_PREPARE.TRAIN_IMAGE_DIR, series_id + '_0000.nii.gz')
                mask_path = os.path.join(self.cfg.DATA_PREPARE.TRAIN_MASK_DIR, series_id + '.nii.gz')
                ...
                if os.path.exists(image_path):
                    ...
                    print('%s has invalid image.' % series_id)
        for series_id in series_ids_u:
            if phase == 'train':
                image_path = os.path.join(self.cfg.DATA_PREPARE.TRAINU_IMAGE_DIR, series_id + '_0000.nii.gz')
                #mask_path = os.path.join(self.cfg.DATA_PREPARE.TRAIN_MASK_DIR, series_id + '.nii.gz')
                if os.path.exists(image_path):
                    data_dict = {'image_path': image_path,
                    'mask_path': 'None'}
                    DataBaseUtils.update_record_in_db(dbu_file_path, series_id, data_dict)
                else:
                    print('%s has invalid image/mask.' % series_id)
    ...
    def _split_train_val(self):
        ...
        default_trainu_db = self.cfg.DATA_PREPARE.DEFAULT_TRAINU_DB
        ...
        if default_trainu_db is not None:
            env = lmdb.open(default_trainu_db, map_size=int(1e9))
            txn = env.begin()
            series_ids_trainu = []
            for key, value in txn.cursor():
                key = str(key, encoding='utf-8')
                series_ids_trainu.append(key)
            env = lmdb.open(self.outu_db_file, map_size=int(1e9))
            txn = env.begin()
        else:
            env = lmdb.open(self.outu_db_file, map_size=int(1e9))
            ...
            for key, value in txn.cursor():
                key = str(key, encoding='utf-8')
                series_u_ids.append(key)

        print('Num of train u series is: %d.' % (len(series_u_ids)))

        # create train db
        env_train = lmdb.open(self.out_trainu_db_file, map_size=int(1e9))
        txn_train = env_train.begin(write=True)
        for series_id in series_u_ids:
            value = str(txn.get(str(series_id).encode()), encoding='utf-8')
            txn_train.put(key=str(series_id).encode(), value=str(value).encode())
        txn_train.commit()
        env_train.close()
